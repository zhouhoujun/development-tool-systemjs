...
var SystemjsBundle = (function (_super) {
    ...
    SystemjsBundle.prototype.loadBuilder = function (ctx) {
        ...
        var jsbuilder = new Builder('./', _.isArray(option.systemConfig) ? _.first(option.systemConfig) : option.systemConfig);
    ...
    SystemjsBundle.prototype.initBundles = function (ctx) {
        ...
        if (opt.bundleDeps) {
            pr = pr.then(function (bundles) {
                ...
                var deps = opt.dependencies ? ctx.to(opt.dependencies) : _.keys(pkg.dependencies);
    ...
    SystemjsBundle.prototype.initOption = function (ctx) {
        var option = _.extend({}, {
            ...
            bundlePaths: function (ctx) {
                ...
                var npmpkg = ctx.toRootPath('./node_modules'); // ctx.getPackage()[''];
                var jp = path.basename(npmpkg) + '/*';
    ...
    SystemjsBundle.prototype.getAssertResetPipe = function (ctx) {
        if (!this.restps) {
            ...
            if (option.resetAsserts) {
                ...
                var urlprefix1_1 = "url\\(\\'";
                var urlprefix2_1 = 'url\\(\\"';
                var urlprefix3_1 = 'url\\(';
                var urlprefix4_1 = 'url\\(\\\\"';
                var urlprefix5_1 = "url\\(\\\\'";
                var urlpath_1 = '\\.\\.\\/';
                _.each(folders, function (f) {
                    var relp = ctx.toUrl(path.join(baseURL_1, ctx.toUrl(root_1, ctx.toUrl(dist_1, f))));
                    var fm = path.basename(f);
                    console.log('reset css url folder name:', chalk.cyan(fm), 'relate url:', chalk.cyan(relp));
                    var reg = new RegExp("(" + urlprefix3_1 + "(" + urlpath_1 + ")+" + fm + ")|(" + urlprefix3_1 + "\\/" + fm + ")", 'gi');
                    ps_1.push(function () { return replace(reg, "url(" + relp); });
                    var reg2 = new RegExp("(" + urlprefix5_1 + "(" + urlpath_1 + ")+" + fm + ")|(" + urlprefix5_1 + "\\/" + fm + ")", 'gi');
                    ps_1.push(function () { return replace(reg2, "url(\\'" + relp); });
                    var reg3 = new RegExp("(" + urlprefix2_1 + "(" + urlpath_1 + ")+" + fm + ")|(" + urlprefix2_1 + "\\/" + fm + ")", 'gi');
                    ps_1.push(function () { return replace(reg3, "url(\"" + relp); });
                    var reg4 = new RegExp("(" + urlprefix1_1 + "(" + urlpath_1 + ")+" + fm + ")|(" + urlprefix1_1 + "\\/" + fm + ")", 'gi');
                    ps_1.push(function () { return replace(reg4, "url('" + relp); });
                    var reg5 = new RegExp("(" + urlprefix4_1 + "(" + urlpath_1 + ")+" + fm + ")|(" + urlprefix4_1 + "\\/" + fm + ")", 'gi');
                    ps_1.push(function () { return replace(reg5, "url(\\'" + relp); });
                });
                this.restps = ps_1;
            }
            else {
