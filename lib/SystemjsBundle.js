    ...
    SystemjsBundle.prototype.getAssertResetPipe = function (ctx) {
        if (!this.restps) {
            ...
            if (option.resetAsserts) {
                ...
                var urlprefix1_1 = "url\\(\\'(\\/){0,1}";
                var urlprefix2_1 = 'url\\(\\"(\\/){0,1}';
                var urlprefix3_1 = 'url\\((\\/){0,1}';
                var urlprefix4_1 = 'url\\(\\\\"(\\/){0,1}';
                var urlprefix5_1 = "url\\(\\\\'(\\/){0,1}";
                var urlpath_1 = '\\.\\.\\/';
                _.each(folders, function (f) {
                    ...
                    var reg1 = new RegExp("(" + urlprefix1_1 + "(" + urlpath_1 + ")+" + fm + ")|(" + urlprefix1_1 + "\\/" + fm + ")", 'gi');
                    ps_1.push(function () { return replace(reg1, "url('" + relp); });
                    var reg2 = new RegExp("(" + urlprefix2_1 + "(" + urlpath_1 + ")+" + fm + ")|(" + urlprefix2_1 + "\\/" + fm + ")", 'gi');
                    ps_1.push(function () { return replace(reg2, "url(\"" + relp); });
                    var reg3 = new RegExp("(" + urlprefix3_1 + "(" + urlpath_1 + ")+" + fm + ")|(" + urlprefix3_1 + "\\/" + fm + ")", 'gi');
                    ps_1.push(function () { return replace(reg3, "url(" + relp); });
                    var reg4 = new RegExp("(" + urlprefix4_1 + "(" + urlpath_1 + ")+" + fm + ")|(" + urlprefix4_1 + "\\/" + fm + ")", 'gi');
                    ps_1.push(function () { return replace(reg4, "url(\\\"" + relp); });
                    var reg5 = new RegExp("(" + urlprefix5_1 + "(" + urlpath_1 + ")+" + fm + ")|(" + urlprefix5_1 + "\\/" + fm + ")", 'gi');
